...
@php
    ...
    if ($rows === null) {
        $rows = $customFields->map(fn ($field) => [
            ...
            'is_required' => $field->obbligatorio,
            'options_text' => $field->options->pluck('valore')->implode("\n"),
        ])->values()->all();
    }
                        ...
                        <form method="post" novalidate>
                            ...
                            <div class="row g-3">
                                <div class="col-md-6 field-group form-group">
                                    ...
                                    <input class="form-control @error('title') is-invalid @enderror" id="title" name="title" maxlength="255" value="{{ old('title', $event->titolo) }}" required>
                                    @error('title')<div class="invalid-feedback d-block">{{ $message }}</div>@enderror
                                </div>
                                <div class="col-md-6 field-group form-group">
                                    <label class="form-label" for="venue_name">Luogo</label>
                                    <input class="form-control @error('venue_name') is-invalid @enderror" id="venue_name" name="venue_name" value="{{ old('venue_name', $event->nome_luogo) }}" required>
                                ...
                                <div class="col-12 field-group form-group">
                                    ...
                                    <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description" rows="5" required>{{ old('description', $event->descrizione) }}</textarea>
                                    @error('description')<div class="invalid-feedback d-block">{{ $message }}</div>@enderror
                                </div>
                                <div class="col-12 field-group form-group">
                                    <label class="form-label" for="venue_address">Indirizzo</label>
                                    <textarea class="form-control @error('venue_address') is-invalid @enderror" id="venue_address" name="venue_address" rows="2" required>{{ old('venue_address', $event->indirizzo_luogo) }}</textarea>
                                    @error('venue_address')<div class="invalid-feedback d-block">{{ $message }}</div>@enderror
                                </div>
                                <div class="col-12 field-group form-group">
                                    <label class="form-label" for="notes">Note</label>
                                    <textarea class="form-control @error('notes') is-invalid @enderror" id="notes" name="notes" rows="3">{{ old('notes', $event->note) }}</textarea>
                                    @error('notes')<div class="invalid-feedback d-block">{{ $message }}</div>@enderror
                                </div>
                                <div class="col-md-6 field-group form-group">
                                    <label class="form-label" for="max_participants">Numero massimo di partecipanti</label>
                                    <input class="form-control @error('max_participants') is-invalid @enderror" id="max_participants" name="max_participants" type="number" min="1" value="{{ old('max_participants', $event->max_partecipanti) }}" required>
                                ...
                                <div class="col-md-6 field-group form-group">
                                    ...
                                    <input class="form-control @error('price') is-invalid @enderror" id="price" name="price" type="number" min="0" step="0.01" value="{{ old('price', $event->prezzo ?? 0) }}" required>
                                    @error('price')<div class="invalid-feedback d-block">{{ $message }}</div>@enderror
                                </div>
                                @foreach (['start_datetime' => ['label' => 'Data e ora di inizio', 'attribute' => 'inizio_il'], 'end_datetime' => ['label' => 'Data e ora di fine', 'attribute' => 'fine_il'], 'registration_deadline' => ['label' => 'Termine iscrizioni', 'attribute' => 'scadenza_iscrizioni']] as $name => $meta)
                                    <div class="col-md-6 field-group form-group">
                                        <label class="form-label" for="{{ $name }}">{{ $meta['label'] }}</label>
                                        @php $dateValue = old($name, $event->{$meta['attribute']} ? $event->{$meta['attribute']}->format('Y-m-d\TH:i') : ''); @endphp
                                ...
                                <div class="col-md-6 field-group form-group">
                                    ...
                                    <select class="form-select @error('event_type') is-invalid @enderror" id="event_type" name="event_type" required>
                                        @foreach ($eventTypes as $value => $label)
                                            <option value="{{ $value }}" @selected(old('event_type', $event->tipo_evento) === $value)>{{ $label }}</option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-md-6 field-group form-group">
                                    ...
                                    <select class="form-select @error('status') is-invalid @enderror" id="status" name="status" required>
                                        @foreach ($statusChoices as $value => $label)
                                            <option value="{{ $value }}" @selected(old('status', $event->stato ?: 'draft') === $value)>{{ $label }}</option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="custom-field-summary-grid">
                                    @forelse ($customFields as $customField)
                                        <article class="custom-field-summary-card">
                                            <h3 class="h6 mb-1">{{ $customField->etichetta }}</h3>
                                            <div class="small text-muted">{{ $customField->field_type_label }} &middot; Ordine {{ $customField->ordine_visualizzazione }} &middot; {{ $customField->obbligatorio ? 'Obbligatorio' : 'Facoltativo' }}</div>
                                            @if ($customField->tipo_campo === 'select')
                                                <ul class="mb-0 mt-2 small text-muted">
                                                    @foreach ($customField->options as $option)<li>{{ $option->valore }}</li>@endforeach
                                                </ul>
                                            @endif
                                        </article>
                                    @empty
